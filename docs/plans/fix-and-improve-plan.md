...
#### ~~任务 15：看板/日历/完成视图统一玻璃拟态风格~~ ✅ 已完成

**问题**：这三个视图还是旧的朴素 Tailwind 风格，与主界面不统一。

**完成情况**：commit `058a9e4a` 已将看板/日历/完成视图改为玻璃拟态风格。待办项：日期解析逻辑仍分散在多个文件中，未统一为 `utils/date.ts`（纳入任务 14 一起处理）。
...
以下改进不在原计划中，但已由其他 Agent 实现：

| 改进 | Commit |
|------|--------|
| ProjectPicker/TagPicker 添加后自动选中新项目/标签 | `309c96ae` |
| ProjectPicker/TagPicker 添加后立即显示在列表中 | `3f256866` |
| 右侧面板副标题跟随 tab 切换显示不同统计 | `8d3cfd7d` |
| 看板按钮添加 cursor-pointer | `e4d98bef` |
| TaskCard 编辑区增加等待日期字段 | `e4d98bef` |
| 视图编辑跳转待办并展开 | `71072d2a` |
| 列表改名"待办" | `71072d2a` |
